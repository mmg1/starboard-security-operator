...
func (a *action) newScanJobContainer(podContainer core.Container) core.Container {
	return core.Container{
		...
		Image: fmt.Sprintf("aquasec.azurecr.io/scanner:%s", a.config.ScannerAquaCSP.Version),
		Command: []string{
			"/bin/sh",
			"-c",
			fmt.Sprintf("/opt/aquasec/scannercli scan --checkonly --host %s --user %s --password %s --local %s 2> %s",
				a.config.ScannerAquaCSP.Host,
				a.config.ScannerAquaCSP.User,
				a.config.ScannerAquaCSP.Password,
				podContainer.Image,
